...
$(function () {
  ...
  var $top_left = $('#top_left') //轮播的显示div
  var $top_left_img = $('#top_left_img')		 //包含img的div
  var $All_a = $('#img_a_nav>a')	//导航的小圆点
  var $prev = $('#prev')				//点击一下回退上一页
  var $next = $('#next')				//点击一下跳到下一页
  var PAGE_WIDTH = 520 //一页的宽度
  var TIME = 400 // 翻页的持续时间
  var ITEM_TIME = 20 // 单元移动的间隔时间
  var imgCount = $All_a.length  //图片的个数
  var index = 0 //当前下标
  ...
  function nextPage (next) {
    ...
    // 得到当前的left值
    var currLeft = $top_left_img.position().left
    // 计算出目标处的left值
    var targetLeft = currLeft + offset

    // 启动循环定时器不断更新$top_left_img的left, 到达目标处停止停止定时器
    var intervalId = setInterval(function () {
      // 计算出最新的currLeft
      currLeft += itemOffset
      ...
      $top_left_img.css('left', currLeft)
    }, ITEM_TIME)
  ...
  function updatePoints (next) {
    ...
    $All_a[index].className = ''
    // 给目标圆点添加class='on'
    // $points.eq(targetIndex).addClass('on')
    $All_a[targetIndex].className = 'a_index'
